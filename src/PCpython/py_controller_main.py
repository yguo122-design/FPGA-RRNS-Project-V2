...
class FpgaController:
    ...
    def receive_response(self) -> Optional[Dict]:
        ...
        try:
            # Read fixed total length of 2011 Bytes
            raw_data = self.serial_conn.read(FRAME_LEN_RESP)
            
            if len(raw_data) < FRAME_LEN_RESP:
                print(f"[ERROR] Timeout or incomplete data! Expected {FRAME_LEN_RESP} bytes, received {len(raw_data)} bytes")
                return None
            ...
            if raw_data[0:2] != HEADER_RESP:
                print(f"[ERROR] Header mismatch: Expected {HEADER_RESP.hex()}, received {raw_data[0:2].hex()}")
                return None
            ...
            # Doc example shows 0x077A (1914), but we parse based on actual structure.
            len_field = struct.unpack('>H', raw_data[3:5])[0]
            # print(f"   Length Field Value: {len_field} (0x{len_field:04X})")
...
def main():
    ...
    # Uses default port. Can be modified in constants or via CLI args if needed.
    port = DEFAULT_PORT
    
    controller = FpgaController(port=port, baudrate=DEFAULT_BAUDRATE)
