...
│   │   │   └── 离线计算 91点×4算法×15突发长度 的阈值表 -> threshold_table.coe
│   │   │
│   │   ├── [Execution] 测试执行引擎
...
*   **91 点循环扫描 (`Main SCAN FSM`)**：
    ...
    *   **索引递增**：当单点采样数达到 $N$ 时，FSM 自动保存该点统计结果，`ber_idx++`，切换至下一个 BER 阈值，直至完成全部 **91 个测试点**。
*   **测试终止**：91 个点全部完成后，FSM 自动拉低 `test_active` 停止测试，并触发上报信号。

#### **3. 结果回传阶段 (Reporting Phase)**
*   **数据组装 (`tx_packet_assembler`)**：
    *   FSM 控制读取片上 RAM (`mem_stats_array`) 中缓存的 91 组统计数据（包含成功/失败次数、时钟计数、实际翻转数等）。
    *   添加全局信息头（算法 ID、总点数等）及帧尾校验和，组装成约 **1.5KB** 的完整响应帧。
*   **上行传输**：通过 `uart_tx_module` 将二进制流发回 PC。
*   **解析与可视化**：
    *   PC 端驱动接收并校验数据帧。
    *   应用层解析二进制数据，计算各点 BER，绘制 **BER 性能曲线图**，并支持导出 CSV 报告。
    * 对于各个算法之间的汇总对比，需要由单词的python程序读取CSV进行输出（因为每次与FPGA单板通讯测试只能测试一种算法的数据）

---

### **💡 设计亮点总结 (Design Highlights)**
1.  **极简协议**：下行仅需 7 字节有效载荷，去除了复杂的阈值计算与启停握手，降低通信开销。
2.  **全自动化**：FPGA 内部集成 ROM 查表与状态机调度，单次命令即可完成全谱系（91 点）扫描，极大缩短测试周期。
...
*   **Main SCAN FSM**：对应流程中的 **阶段 2 (核心调度)**，负责 91 点循环与阈值切换。
...
stateDiagram-v2
    ...
    NEXT_BER --> LOAD_THR : BER_Idx < 91
    NEXT_BER --> SEND_REPORT : BER_Idx == 91
...
| 方向           | 模式     | 频率             | 数据内容                                                              | 设计目的                                      |
| :----------- | :----- | :------------- | :---------------------------------------------------------------- | :---------------------------------------- |
| 下行 (PC→FPGA) | 单次配置命令 | 极低频 (每次测试 1 帧) | 仅包含 `Algo`, `Mode`, `Burst_Len`, `Sample_Count`。不再下发 BER 阈值。      | 极简指令。FPGA 收到后自动锁存种子，并内部启动 91 点 BER 全自动扫描。 |
| 上行 (FPGA→PC) | 终结大包回复 | 低频 (每次测试 1 帧)  | 包含 91 个 BER 点 的完整统计数据 (`Success/Fail Count`) 及 `Total_Clk_Count`。 | 避免中间过程频繁上报占用带宽。利用 FPGA 高速计算能力，一次性回传所有结果。  |
...
- **下行命令帧 (Command Frame)**: 固定 **12 Bytes**。
  ...
  - 特点: 紧凑、二进制、含校验。
  - 具体见2.1.3.1章节
- **上行响应帧 (Response Frame)**: 固定 **2011 Bytes**。
  - 结构: `Header(2)` + `CmdID(1)` + `Len(2)` + `GlobalInfo(3)` + `PerPointData(91×22)` + `Checksum(1)` 。
  - 帧长计算: `2+1+2+3+(91×22)+1 = 2011 Bytes`（每点 **22 Bytes**，含 1B Reserved）。
  - 特点: 大数据量、包含 91 个点详情、含头尾标识。
  - 具体见2.1.3.2章节

#### 1.6.3 异常处理与超时保护 (Exception Handling & Timeout)

鉴于上行帧长度为 **2011 Bytes**，经测算：
...
1.  **用户交互接口**:
    ...
    - **输入参数**:
        - `Algo_Type`: 算法选择 (0: 2NRM-RRNS, 1:3NRM-RRNS,  2: C-RRNS, 3: RS)。
...
| **5**       | `cfg_algo_ID`      | 1 Byte   | Uint8  | 算法 ID (0:2NRM, 1:3NRM, 2:C-RRNS, 3:RS)                           |
| **6**       | `cfg_error_mode`   | 1 Byte   | Uint8  | 模式 ID (0:Single, 1:Burst, 2:Fixed)                               |
| **7**       | `cfg_sample_count` | 4 Bytes  | Uint32 | **每个 BER 点**的测试样本数 (Big-Endian)。总测试次数 = `cfg_sample_count` × 91。 |
...
*   **步骤 3**: **Main Scan FSM 进入循环**:
    ...
    *   重复直到 `ber_index` == 91。
*   **步骤 4**: 91 点全部完成后，FSM 拉低 `test_active`，触发 `test_done_flag`，打包上报。
...
| **5**       | **Global Info**                    |             |            |                                                               |
| 5           | `Total_Points`                     | 1 Byte      | Uint8      | 测试的 BER 点数 ( 91，从1%~10%，步长为0.1%)                              |
| 6           | `Algo_Used`                        | 1 Byte      | Uint8      | 算法 ID (0~3)                                                   |
| 7           | `Mode_Used`                        | 1 Byte      | Uint8      | 模式 ID (0/1)                                                   |
| **8**       | **Per-Point Data** (Loop 91 times) |             |            | 每个 BER 点占 22 Bytes                                            |
| +0          | `BER_Index`                        | 1 Byte      | Uint8      | 当前点索引 (0~90)                                                  |
...
- **全局停止**: 仅当接收到 FSM 发出的 test_done_flag (表示 91 个点全部完成) 时，才将 test_active 清零，结束测试。
...
| **sys_test_done**    | Output | 1      | Wire  | 测试完成脉冲。当 91 个点全部扫描结束时拉高 1 个时钟周期。                                         |
 ...
 **任务级锁存机制**：`lock_seed` 信号仅在 `Main Scan FSM` 进入 **`INIT`** 状态时拉高，并在整个 91 个 BER 测试点扫描完成、FSM 进入 **`DONE`** 状态前保持有效。
**目的**：确保 91 个不同误码率测试点的数据均源自同一随机序列片段，消除因种子重置导致的统计偏差，保证跨测试点数据的一致性。
...
module seed_lock_unit (
    ...
    // Control
    input  wire        lock_en,       // High during entire 91-point scan (From FSM INIT state)
...
2.  **目标 BER 索引 (BER_Index)**: 0~90 → 对应 $$ BER_{target} = 0.01 + (index \times 0.001) $$**突发长度 (Burst_Len)**: 1~15

**查找公式**：
$$ \text{Addr} = (\text{Algo} \times 91 \times 15) + (\text{BER\_Index} \times 15) + (\text{Burst\_Len} - 1) $$

*   **表深度 (Depth)**: $4 \times 91 \times 15 = 5460$ 个条目。
...
**2. ROM 地址映射逻辑**
	...
	*   **高位 [11:10]**：`algo_id` (2-bit)，区分 4 种编码算法。
...
module error_injector (
    ...
    input wire [1:0] algo_id,      // 算法选择: 0=RS, 1=C-RRNS, 2=3NRM, 3=2NRM
...
module rom_threshold_ctrl (
    ...
    input  wire [1:0]  i_algo_id,     // 0~3: RS, C-RRNS, 3NRM, 2NRM
...
module decoder_wrapper (
    ...
    // Algorithm Selection
    input  wire [1:0]  algo_sel,      // 0:RS, 1:C-RRNS, 2:3NRM, 3:2NRM
...
**设计目标**：
本模块负责在 FPGA 完成 **全量程 BER 扫描（91 个点）** 后，将存储在片上 RAM 中的统计数据打包成 **单帧大数据包**，并通过 UART 接口一次性发送给 PC 端。**所有 BER 计算、曲线绘制及性能分析均在 PC 端 Python 脚本中完成**，FPGA 仅作为高精度数据采集器。

**1. 工作流程 (基于命令触发)**

1.  **空闲态**：FPGA 等待 PC 下发 配置（采用下发配置即启动的模式）。
2.  **扫描态**：收到命令后，Main SCAN FSM 控制 Auto Scan Engine 依次遍历 91 个 BER 点 ($10^{-2} \sim 10^{-1}$, step $10^{-3}$)。
    *   每个点的测试结果 (`Success`, `Fail`, `Flip_Count`, `Clk_Count`) 实时写入 **`mem_stats_array`**。
3.  **完成态**：91 个点全部测试完毕后，FSM 拉高 `scan_done` 信号。
4.  **上报态**：`tx_packet_assembler` 被激活，从 `mem_stats_array` 读取全部 91 个点的数据，按照 **Table 2-X (FPGA->PC Protocol)** 组装成大帧，通过 UART 发送。
5.  **返回空闲**：发送完成后，等待下一条命令。

**2. 硬件架构与存储规划**

##### **A. mem_stats_array (大容量统计缓存 BRAM)**
由于需要缓存 91 个点的完整数据，且每个点包含 64-bit 时钟计数，存储需求较大。
*   **单点数据结构 (21 Bytes)**：
...
*   **总存储需求**：$91 \times 21 \text{ Bytes} = 1911 \text{ Bytes}$。
*   **实现方案**：
...
这是 91 点数据的存储核心，必须明确双端口（Dual Port）行为。
...
| :--- | :--- | :--- | :--- |
| `[175:168]` | `BER_Index` | 8-bit | 当前 BER 点索引 (0~90) |
...
*   **帧结构总览**：
    ...
    *   **Per-Point Data**: $91 \times 22 = 2002$ Bytes (每点 **22 Bytes**，含 1B Reserved)
    *   **Checksum**: 1 Byte
...
// Logic: State machine to generate Header -> Global Info -> Loop 91 times (Break 168-bit to 21 bytes) -> Checksum
endmodule
```
 ...
 *   **影响**: 总报告帧长由 2011 Bytes 微调至 **2011 Bytes** (91 points × 22 Bytes + Header/Checksum)。传输耗时增加 < 0.2ms，可忽略不计。
...
| `input` | `algo_id` | 2 | Constant | **算法选择ID**。虽为输入，但在单算法编译策略下由顶层固定传入 (`CURRENT_ALGO_ID`)。`0`: 2NRM, `1`: 3NRM, `2`: C-RRNS, `3`: RS |
...
**目的**：
- 防止 `test_active` 持续高电平期间 FSM 被重复触发，确保每次配置下发只启动**一轮** 91 点扫描。
...
. **协议优化**：明确上行链路帧结构，计算并确认精简版帧长为 2011 Bytes（91 点测试），提升传输效率。
. **时序保障**：明确采用 ROM 查表法 预存 BER 阈值，消除动态计算带来的时序收敛风险。
. **异常处理**：新增看门狗联动机制，定义解码器死锁时的复位与状态机跳转逻辑，防止系统挂死。
. 将 Seed Lock 触发机制由“**单点触发**”调整为“**任务级锁存**”。明确 lock_seed 信号覆盖整个 91 点扫描周期，以确保统计一致性。
...
3.  **工作模式变更：从“流式”转为“缓存 - 爆发”模式**：
    ...
    *   **新方案**：FPGA 独立跑完全部 91 个 BER 点 ($10^{-2} \sim 10^{-4}$)，将结果暂存于片上 BRAM，测试结束后**一次性打包上传**。
...
4.  **协议帧结构严格对齐与修正**：
    ...
    *   **帧长度计算**：
        *   Global Info: 3 Bytes
        *   Per-Point Data: $91 \times 22 = 2002$ Bytes
...
#### **🛡️ 可靠性与资源设计**
5.  **大容量统计缓存设计**：
    *   规划 **2002 Bytes** 的专用 Block RAM (`mem_stats_array`)，采用双端口配置（测试时写入，上报时读取），支持 91 个点的全量数据暂存。
...
*   **`tx_packet_assembler`:** Defined the state machine interface for reading BRAM and handling UART **backpressure** (`tx_ready`), ensuring robust 2011-byte frame transmission without packet loss.
*   **`seed_lock_unit`:** Clarified the `lock_en` (task-level) and `capture_pulse` (config-level) signals to ensure deterministic random seed generation across all 91 test points.
...
**修改3 - 帧长计算公式修正（Section 2.3.3.5 B部分）**：明确 Length 字段值为 **2005 (0x07D5)**，每点 **22 Bytes**，总帧长公式为 `2+1+2+3+(91×22)+1 = 2011 Bytes`，消除了旧文档中"21 Bytes per point"的遗留错误。
