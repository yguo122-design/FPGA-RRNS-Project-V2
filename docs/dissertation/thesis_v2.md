![](figure/Dissertation_Cover.png){ width=100% }
...
![Figure 3.1 Top-level system architecture of the FPGA fault-tolerance evaluation platform|540](figure/system_architecture.drawiov2.png){ width=443px }
...
![285](figure/state%20diagram.png){ width=200px }
...
| Algo\_ID | Algorithm            | Decoder Architecture                        | Codeword (bits) | Error Correction      |
| -------- | -------------------- | ------------------------------------------- | --------------- | --------------------- |
| 0        | 2NRM-RRNS (Parallel) | 15-channel parallel MLD pipeline            | 41              | t = 2 residues        |
| 1        | 3NRM-RRNS            | Sequential FSM MLD, 84 triplets             | 48              | t = 3 residues        |
| 2        | C-RRNS-MLD           | Sequential FSM MLD, 84 triplets             | 61              | t = 3 residues        |
| 3        | C-RRNS-MRC           | Direct Mixed Radix Conversion               | 61              | None (detection only) |
| 5        | RS(12,4)             | Berlekamp–Massey + Chien + Forney           | 48              | t = 4 symbols         |
| 6        | 2NRM-RRNS (Serial)   | Sequential FSM MLD, 15 pairs × 5 candidates | 41              | t = 2 residues        |

C-RRNS-CRT was excluded from the final benchmark because it is functionally redundant with C-RRNS-MRC as a non-correcting baseline and would not add a meaningful new point to the design space. The most important controlled comparison retained in the final platform is the 2NRM-RRNS parallel/serial pair, which isolates the resource-latency trade-off of parallel MLD for the same 41-bit code and the same decoding rule. Algorithm-specific pseudocode is provided in Appendices B–G.
...
![Figure 4.1 Decode success rate vs. actual BER — Random single-bit injection (FPGA vs MATLAB)](figure/fpga_vs_matlab_random_single_bit_20260407_155154.png){ width=494px }
...
Figure 4.2 shows the decode success rate as a function of actual injected BER for all algorithm configurations under cluster burst injection with burst length L=12. This burst length was selected as the representative case because it is long enough to challenge all algorithms — including C-RRNS-MLD — while remaining within the valid injection range for all codeword widths considered in this work. Both FPGA hardware measurement results and MATLAB simulation results are shown side by side.

![Figure 4.2 Decode success rate vs. actual BER — Cluster burst injection, L=12 (FPGA vs MATLAB)](figure/fpga_vs_matlab_cluster_burst_len12_20260407_155154.png){ width=496px }
...
- **C-RRNS-MLD** (L=14): t=3 correction over wide redundant moduli (6–7 bits/residue) keeps a 14-bit burst within 2–3 corrupted residues, within the correction limit.
...
- **3NRM-RRNS-MLD** (L=11): despite the same t=3 as C-RRNS, smaller redundant moduli (4–5 bits/residue) cause a given burst to span more residue boundaries, reducing practical burst tolerance.
...
![](figure/latency_comparison.png){ width=522px }

**Figure 4.3** Average encoder and decoder latency comparison (clock cycles at 50 MHz, log scale). 

The figure uses a logarithmic scale to visualise the two-orders-of-magnitude range; precise numerical values are provided in Table 4.3.
...
![](figure/utilization_comparison.png){ width=547px }
...
![](figure/MATLAB_simulation_architecture.png){ width=697px }
...
![decoding success rate](figure/decoding_performance_mrc.png){ width=419px }
...
![](figure/decoding_time_mrc.png){ width=461px }
...
3. **The latency ranking** (fastest to slowest) is: RS ≈ 2NRM-RRNS ≈ 3NRM-RRNS ≪ C-RRNS (MRC). This is consistent with the FPGA hardware results presented in Section 4.4, where C-RRNS-MLD (928 cycles) is one of the slower correcting decoders, 3NRM-RRNS reaches 2048 cycles, and 2NRM-RRNS Parallel (24 cycles) is the fastest among all error-correcting decoders (noting that C-RRNS-MRC achieves 9 cycles but provides no error correction capability).

## Appendix I: Hierarchical Module Structure of FPGA platform

The system is organised into two major domains — a PC-side Python application and an FPGA-side Verilog implementation — as shown in the following table.
...
![Figure K.1 Cluster length impact — C-RRNS-MLD](figure/cluster_length_impact_C_RRNS_MLD_20260328_193838.png){ width=450px }

**Figure K.1** Decode success rate vs. BER for C-RRNS-MLD at burst lengths L = 5, 8, 12, 13, 14, 15.

![Figure K.2 Cluster length impact — RS(12,4)](figure/cluster_length_impact_RS_20260328_193838.png){ width=466px }

**Figure K.2** Decode success rate vs. BER for RS(12,4) at burst lengths L = 5, 8, 12, 13, 14, 15.

![Figure K.3 Cluster length impact — 3NRM-RRNS](figure/cluster_length_impact_3NRM_RRNS_20260328_193838.png){ width=461px }

**Figure K.3** Decode success rate vs. BER for 3NRM-RRNS at burst lengths L = 5, 8, 10, 11, 12, 15.

![Figure K.4 Cluster length impact — 2NRM-RRNS Parallel](figure/cluster_length_impact_2NRM_RRNS_20260329_081944.png){ width=460px }

**Figure K.4** Decode success rate vs. BER for 2NRM-RRNS (Parallel) at burst lengths L = 7, 8, 9, 10, 12, 15.

![Figure K.5 Cluster length impact — 2NRM-RRNS Serial](figure/cluster_length_impact_2NRM_RRNS_Serial_20260328_193838.png){ width=461px }
