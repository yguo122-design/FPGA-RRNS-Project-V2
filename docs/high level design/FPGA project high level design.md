...
- 各个算法生成的codword长度见如下表格（体现了存储效率）![[Pasted image 20260329100059.png]]
- 各个算法在上述模式配置情况下的纠错能力对比见如下表格![[Pasted image 20260329100159.png]]

# 项目第一阶段（MATLAB仿真阶段）综述

在项目的第一阶段采用了MATLAB对各个算法进行了仿真对比，主要的结论见如下：

- 不同误码率的情况下解码成功率对比见如下图![[Pasted image 20260329100227.png]]
从这个图中可以看到，从有误码的情况下解码成功情况看，RS算法与3NRM-RRNS相当（3NRM-RRNS）稍微优于RS算法；C-RRNS排在中间；而2NRM-RRNS的误码情况下解码恢复能力在四种算法里面最低

- 不同解码算法的解码资源消耗（从解码时间来对比）见下图![[Pasted image 20260329100256.png]]![[high level design/system_architecture.drawiov2.png]]
...
This system adopts a **"PC centralized control + FPGA fully automatic execution"** master-slave architecture. The PC side is responsible for high-level strategy configuration and data visualization, while the FPGA side performs microsecond-level high-speed error injection, decoding verification, and statistics reporting. The overall interaction flow is divided into three phases: **configuration delivery**, **auto-scan execution**, and **result reporting**, as detailed below:
...
*   **Data Assembly (`tx_packet_assembler`)**:
    *   The FSM controls reading 101 groups of cached statistics from on-chip RAM (`mem_stats_array`) (including success/failure counts, clock counts, actual flip counts, etc.).
    *   It adds a global information header (algorithm ID, total points, etc.) and frame-tail checksum, assembling a complete response frame of about **2.2KB**.
